...
func fixRow(rules, rrules Rule, row []string) []string {
	row2 := make([]string, len(row))
	copy(row2, row)

	for i := 0; i < len(row)-1; i++ {
		flg := false
		for _, v := range rules[row[i]] {
			if v == row[i+1] {
				flg = true
				break
			}
	...
	if checkRow(rules, rrules, row2) {
		return row2
	}

	return fixRow(rules, rrules, row2)
}
	...
	for sc.Scan() {
		...
		if mode == 0 {
			...
			if !checkRow(rules, rrules, row) {
				nrow := fixRow(rules, rrules, row)
				num, err := strconv.Atoi(nrow[len(nrow)/2])
				if err != nil {
					panic(err)
				}
				fmt.Printf("center: %d\n", num)
